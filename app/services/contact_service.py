...
def _base_contact_values(payload: ContactCreateModel) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {
        ...
        "source": normalize_code(payload.source),
...
def create_contact(
    ...
    if not values.get("source"):
        values["source"] = (
            "COUNSELLOR_MANUAL_ENTRY"
            if actor.get("role") == UserRole.COUNSELLOR.value
            else "ADMIN_MANUAL_ENTRY"
        )
    now = utc_now()
    operation_id = f"contact-create:{uuid.uuid4()}"
...
def get_contact_detail(contact_id_value: Any, actor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "activeLead": find_active_lead_by_contact(contact["_id"]),
    }
...
def list_contacts(
    ...
    *,
    ...
    sort: str = "-createdAt",
) -> Tuple[List[Dict[str, Any]], int, Dict[ObjectId, Dict[str, Any]]]:
    ...
    return documents, total, {item["contactId"]: item for item in preferences}
...
def patch_contact(
    ...
    if "source" in supplied:
        value = normalize_code(payload.source)
        if value is None:
